...
function VideoDetail({ matchId, mediaType, where }: Props) {
  ...
  const { data: detail, isLoading: detailLoading } = useQuery<GetDetail>(['detail', mediaType, matchId], () =>
    contentsApi.getDetails(mediaType, matchId)
  );

  const clickedData = (matchId && detail?.id) === +matchId;

  const { data: credit, isLoading: creditLoading } = useQuery<Credit>(['credit', mediaType, matchId], () =>
    contentsApi.getCredits(mediaType, matchId)
  );

  const { data: recommendations, isLoading: recommendationsLoading } = useQuery<RelatedContent>(
    ['recommendations', mediaType, matchId],
    () => contentsApi.getRecommendations(mediaType, matchId)
  );

  const { data: similar, isLoading: similarLoading } = useQuery<RelatedContent>(['similar', mediaType, matchId], () =>
  ...
  const { data: seasonTV } = useQuery<TVSeason>(['seasonTV', matchId, seasonNum.season], () =>
    contentsApi.getSeasonTV(matchId, seasonNum.season)
  );

  const loading = detailLoading || creditLoading || recommendationsLoading || similarLoading;
  ...
  useEffect(() => {
    if (clickedData && detail && detail?.videos.results.length > 0) {
      const teaser = detail?.videos.results.find((item) => item.type === 'Trailer');
                ...
                {detail && (
                  <>
                    ...
                    <BigInfo>
                      ...
                      <VideoInfo detail={detail} mediaType={mediaType} credit={credit} />
                      ...
                      {mediaType === 'tv' && <NoContents>{seasonTV && <TvSeason seasonApi={seasonTV} />}</NoContents>}
