...
- **영화 페이지 & 시리즈 페이지**
  ...
  - 영화와 시리즈 콘텐츠들을 캐러셀 슬라이드를 만들어 목록을 보여주도록 구현하여 구현하였다. 만들 당시에는 공부하는 입장이기도 하고 원하는 동작의 구현하는 방식을 찾기 위해 구글링을 많이 했었다. 정말 넷플릭스처럼 깔끔하지도 못하고 자잘한 버그가 있지만 그래도 큰 문제 없이 동작한다. (사실 만들 당시에는 능력자들이 미리 만들어 둔 라이브러리들이 존재하는지 몰랐다...)
